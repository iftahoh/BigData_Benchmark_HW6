...
# --- Logging (capture all prints to results/run_output.txt) ---
class _TeeStdout:
...
class BenchmarkRunner:
    ...
    def generate_tpch_data(self, sf, duckdb_path):
        print(f"--- Generating Data SF={sf} ---")
        # Requirement: use a DuckDB file on disk (not :memory:)
    ...
    def execute_and_measure(self, con, query_sql, query_id, config_name, check_validation=False, duck_ref_res=None):
        ...
        for i in range(3):  # הרצה 3 פעמים
            try:
                ...
                last_end_time = datetime.now()  # end time of this run (we will keep the last one)
                times.append(end - start)
            except Exception as e:
                print(f"Error running Q{query_id} on {config_name}: {e}")
                return 9999.0, None, None  # penalty time on failure
        ...
        return median_time, last_result, last_end_time

    def save_result(self, query_id, config, sf, time_val, last_run_timestamp, db_size_bytes):
        row = {
            "Query": query_id,
    ...
    def run_benchmark(self):
        ...
        for i, sf in enumerate(SF_PLAN):
            ...
            # 1. יצירת דאטה
            sf_tag = str(sf).replace('.', 'p')
            duckdb_path = os.path.join('results', f'duck_sf_{sf_tag}.duckdb')
            duck_con = self.generate_tpch_data(sf, duckdb_path)
            ...
            # DB sizes on disk (required to be stored per query)
            db_sizes = {
                'DuckDB': os.path.getsize(duckdb_path) if os.path.exists(duckdb_path) else None,
                'SQLite_No_Index': os.path.getsize('sqlite_no_index.db') if os.path.exists('sqlite_no_index.db') else None,
                'SQLite_With_Index': os.path.getsize('sqlite_with_index.db') if os.path.exists('sqlite_with_index.db') else None,
            }
            ...
            for q_id, q_sql in self.queries.items():
                ...
                if q_id in self.active_queries["DuckDB"]:
                    ...
                    t, duck_res, ts = self.execute_and_measure(duck_con, q_sql, q_id, "DuckDB")
                    # Sample output (required): print a few rows
                    if duck_res is not None:
                        print(f"Sample result Q{q_id} (DuckDB) rows={len(duck_res)}: {duck_res[:3]}")
                    self.save_result(q_id, "DuckDB", sf, t, ts, db_sizes['DuckDB'])
                ...
                if con_lite_no and q_id in self.active_queries["SQLite_No_Index"]:
                    ...
                    t, _, ts = self.execute_and_measure(con_lite_no, q_sql, q_id, "SQLite_No_Index",
                                                    check_validation=do_valid, duck_ref_res=duck_res)
                    self.save_result(q_id, "SQLite_No_Index", sf, t, ts, db_sizes['SQLite_No_Index'])
                ...
                if con_lite_idx and q_id in self.active_queries["SQLite_With_Index"]:
                    ...
                    t, _, ts = self.execute_and_measure(con_lite_idx, q_sql, q_id, "SQLite_With_Index")
                    self.save_result(q_id, "SQLite_With_Index", sf, t, ts, db_sizes['SQLite_With_Index'])
...
if __name__ == "__main__":
    ...
    # Tee all stdout to a text file required by the assignment
    _tee_file = open(os.path.join("results", "run_output.txt"), "w", encoding="utf-8")
    sys.stdout = _TeeStdout(sys.__stdout__, _tee_file)
